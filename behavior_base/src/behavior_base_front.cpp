...
class BehaviorBase {
    protected:
        ...
        void pc_callback(const sensor_msgs::PointCloud2ConstPtr msg) {
            ...
            // At this stage sensor_map[0] corresponds to the closest range in front of
            // the robot, sensor_map[-1] corresponds to the closest range in
            // [-15 : -45] degrees, sensor_map[1] in [15:45], etc.

            // ROS_INFO("Sensor map: %.2f %.2f %.2f %.2f %.2f", sensor_map[-2],sensor_map[-1],
            //         sensor_map[0], sensor_map[1], sensor_map[2]);
            
            try {
                desired_speed.linear.x = std::min(1.0, std::max((sensor_map[0]-safety_range)/(dont_care_range-safety_range),0.0))*max_linear_speed;
                desired_speed.angular.z = std::min(sensor_map[1],1.0) - std::min(sensor_map[-1],1.0);
                ...
                if ((fabs(desired_speed.linear.x)<5e-2) && (fabs(desired_speed.angular.z)<1e-2)) {
                    desired_speed.angular.z = 0.1;
                }
